...
satVariable( villageTower(V) ):- village(V).  % means "there is a tower at position I-J"
...
writeClauses(MaxNumTowers):-
    eachSVoneT,
    eachVcoveredbyatLeastoneT,
    eachVexactlyOneT,
    atMostTowers(MaxNumTowers),
    linkTPtoVT,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachSVoneT:- significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), exactly(1, Lits), fail.
eachSVoneT.

eachVcoveredbyatLeastoneT:- village(V), findall(towerPos(I,J), (posWatchesVillage(V,I,J), village(V2), posVillage(V2,I,J)), Lits), atLeast(1, Lits), fail.
eachVcoveredbyatLeastoneT.

atMostTowers(MaxNumTowers):- findall(villageTower(V), village(V), Lits), atMost(MaxNumTowers,Lits), fail.
atMostTowers(_).

eachVexactlyOneT:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), atMost(1,Lits), fail.
eachVexactlyOneT.

linkTPtoVT:- posVillage(V,I,J), writeClause([-towerPos(I,J), villageTower(V)]), fail.
linkTPtoVT.
...
costOfThisSolution(M,Cost):- findall([I,J],member(towerPos(I,J),M),L), sort(L,L1), length(L1,Cost), !.
