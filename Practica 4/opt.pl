...
satVariable( hasTwoDoubles(S)  ):- team(S).                                   %  "meaning "team S has two doubles."
...
maxTwoDoublesPerTeam:- team(S), findall(double(S,R), round(R), Lits), atMost(2,Lits), fail.
maxTwoDoublesPerTeam.

maxOneTvMatchFirstTenRounds:- round(R), R =< 10,
                              findall(match(S,T,R), tvMatch(S,T), Lits),
                              atMost(1,Lits), fail.
maxOneTvMatchFirstTenRounds.

%modify this:
maxDoubles(Max):- findall(hasTwoDoubles(S), team(S), Lits), atMost(Max,Lits), fail.
maxDoubles(_).


twoDoublesImpliesHasTwoDoubles:- team(T), round(R), round(R1), R \= R1,
...
% solution of this exam:
costOfThisSolution(M,Cost):- findall(S, member(hasTwoDoubles(S), M), L), length(L,Cost).
