...
writeClauses(infinite):- !, writeClauses(24),!.
writeClauses(MaxConsecutiveHours):-
    notConsecutiveHours,
    notSameHour,
    exactlyNgangstersPerHour,
    gangNotAvailable,
    maxHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

%ok
notConsecutiveHours:- available(G,H),
                      available(G,H1), H1 is H+1,
                      task(T), task(T2), T \= T2,
                      writeClause([-does(G,T,H), -does(G,T2,H1)]), fail.
notConsecutiveHours.

%ok
notSameHour:- gangster(G), hour(H), findall(does(G,T,H), task(T), Lits), atMost(1, Lits), fail.
notSameHour.

%ok
maxHours(MaxConsecutiveHours):- gangster(G), task(T), hour(H1), hour(H2), H2 is H1 + MaxConsecutiveHours,
                    findall( does(G,T,H), between(H1,H2,H), Lits), negateAll(Lits, Nlits),
                    atLeast(1, Nlits), fail.
maxHours(_).

%ok
exactlyNgangstersPerHour:- needed(T,H,N), findall(does(G,T,H), gangster(G), Lits),
                           exactly(N,Lits), fail.
exactlyNgangstersPerHour.

%ok
gangNotAvailable:- task(T), blocked(G, H), writeClause([-does(G,T,H)]), fail.
gangNotAvailable.
...
between(0, 71, N), K is 72-N, algunGangsterTreballaConsecutivament(K,M), !.

algunGangsterTreballaConsecutivament(K, M):- gangster(G), gangsterTreballaConsecutivament(G, K, M).

gangsterTreballaConsecutivament(G, K, M):- llistaHoresTreballades(G,M,L), hihaSubllistaHoresConsecutives(K, L).

llistaHoresTreballades(G, M, L):- gangster(G), findall(H, member(does(G,_,H),M),Hours),sort(Hours,L).
 %   llistaHoresTreballades(G,M,L) :- gangster(G), findall(H, member(does(G,_,H),M),Hours), sort(Hours,L).

hihaSubllistaHoresConsecutives(K, L):- append([_,LK,_],L), length(LK, K), horesConsecutives(LK).

%horesConsecutives([X], 1).
%horesConsecutives([Y,Y2], K):- Y is Y2-1, K2 is K-1, horesConsecutives([Y2], K2).
%horesConsecutives([Y|[Y2|L]], K):- Y is Y2-1, K2 is K-1, horesConsecutives([Y2|L],K2).

horesConsecutives([]).
horesConsecutives([_]) :- !.
horesConsecutives([X,Y | LK]):- Y is X+1, horesConsecutives([Y|LK]).
